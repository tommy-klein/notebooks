---
title: "R, a powerful tool to extract and aggregate gridded datasets"
...
```{r eobs_read, message=FALSE, warning=FALSE}
require(raster)
require(magrittr)
...
Plotting the first 6 records (1950-01-01 - 1950-01-06):
```{r eobs_plot, message=FALSE, warning=FALSE}
plot(tmax.nc[[1:6]])
```
...
```{r shp, message=FALSE, warning=FALSE}
ISO <- 'FRA'
...
```{r lookup, message=FALSE, warning=FALSE}
...
The extraction of grid time series from a RasterBrick is fairly straighforward in R, by subsetting the RasterBrick with a SpatialPoints object (in this case the grid definition). This operation can be quite time consuming. The resulting data structure is a matrix, which is then transposed, converted to a data.frame (long format).
...
```{r ts2, message=FALSE, warning=FALSE}
...
```{r crit, message=FALSE, warning=FALSE}
threshold <- 34.  # degC
...
```{r plot_hist, message=FALSE, warning=FALSE}
